...
function [PS_DR9_analytical,H_N_Pro_succ,P_F] = DR9_analytical (N,pkct_p_h,Header_duration,F_duration,Last_fragment_duration,fragment_length,Header_N_DR9,Threshold,OBW_channels)

%% Probability of header success
% 1. Header collision with header
H_active_2TH = ((N*pkct_p_h*(Header_N_DR9))/3600)*2*Header_duration;
% 2. Header collision with payload data fragments
H_active_TF_TH = ((N*pkct_p_h*(fragment_length-Header_N_DR9-1-1))/3600)*(Header_duration+F_duration);
...
H_N_Pro_succ = 1-(1-H_probability_succ).^Header_N_DR9;

%% Fragments success probability, even if 1/3 (for DR9) of the fragments are lost

%% Payload data Fragment collisions
%1. Fragments collision with fragments
F_active_12_TF = ((N*pkct_p_h*(fragment_length-Header_N_DR9-1-1))/3600)*2*F_duration;
%2. Fragments collision with headers
F_active_12_TH = ((N*pkct_p_h*(Header_N_DR9))/3600)*(Header_duration+F_duration);
...
F_active_last_TH = ((N*pkct_p_h*(Header_N_DR9))/3600)*(Last_fragment_duration+Header_duration);
% 3. Last fragment collision with payload data fragments
F_active_last_TF = ((N*pkct_p_h*(fragment_length-Header_N_DR9-1-1))/3600)*(F_duration+Last_fragment_duration);
% Total colliding packet elements
...
Total_fragment = fragment_length-Header_N_DR9-1;
Frag_avg_suc = (sum(Frag_probability_succ*ones(1,(fragment_length-Header_N_DR9-2)))+Frag_probability_succ_last)/(Total_fragment);
...
PS_DR9_analytical = H_N_Pro_succ*sum(Fragment_success(Threshold:1:Total_fragment)); 
end