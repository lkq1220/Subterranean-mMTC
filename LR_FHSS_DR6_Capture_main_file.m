...
Header_N_DR9 = 2;       % Header replicas
Code_Rate = 2/3;
Header_duration = 0.233472; %233 ms long headers
F_duration = 0.1024;       %102 ms payload data fragments
Header_ToA_DR9 = Header_N_DR9*Header_duration;

[Grid, Center_found,D] = Farm_Coordinates();
[Min_distance, Satellite_subpoint]=min(D);
Farm_Num = length(Grid(:,1));
Sensors_per_Farm = 100;
Total_sensors = Farm_Num * Sensors_per_Farm;

Simulation_T = 3600; % 1 hour duration
pkct_p_h = 1;      % Packets per hour per end-device
OBW_channels=3120;  % No. of OBW channels
MonteCarlo = 1e5;    % No. of Iterations
M = 4;             % M = 2 for DR8, M=4 for DR9

%% Gains and Pt are converted into linear form

Pt = 10^(28/10)/1000;      % Transmit Power of LoRa 14 dBm
Freq_Band = 915e6;         % 868 MHz (frequency band Europe)
SpeedLight  = 3e8;         % Speed of light
wavelength = SpeedLight/Freq_Band;

Gr=(10.^((35)/10));      %22.6 dBi: Gateway at Satellite
...
Distance= Distance([1 45 58 66 72 76 79 81]);
...
k=k([1 45 58 66 72 76 79 81]);

%% Time on air
[ToA_DR9,ToA_DR9_WH] = ToA_Packets_DR9(Payload,Header_ToA_DR9,M); 
% ToA_DR9 -> including headers duration
% ToA_DR9_WH -> without headers duration
Transceiver_wait = 6.472/1000; %Tw      
ToA_DR9(1)=ToA_DR9(1) + Transceiver_wait;  % Total on-air time

%% Number of fragments
fragment_duration = 102.4/1000; %50 ms

fragment_50_ms = floor(ToA_DR9_WH(1)/fragment_duration);  %No. of payload data fragments
% The last fragment may not be equal to 50ms. We need to calculate it.
Last_fragment_duration = ((ToA_DR9_WH(1)/fragment_duration) - fragment_50_ms)*fragment_duration;
if (Last_fragment_duration>0)
    fragment_PHY_length = fragment_50_ms + 1;   
else
    fragment_PHY_length = fragment_50_ms;
    Last_fragment_duration = fragment_duration;
end
% fragment_PHY_length  = fragment_50_ms + 1;
fragment_length = Header_N_DR9 + length(Transceiver_wait) + fragment_PHY_length;

%pack_tx_segments = Header_N_DR9 + ceil((0.102*ceil((Payload+2)/4))/F_duration) +length(Transceiver_wait);
...
H_success = 0;
F_success = 0;
P_snr(c,1) = Probability_SNR (Pt,Gt,Gr,k(c),D_SNR,Distance(c),MonteCarlo,eta,std,RealSoilDielectric, ImagSoilDielectric,Depth);
...
% 
    ...
    seg_trans_simultaneous(:,(Header_N_DR9+1))=[];                            %Twait, that's not a transmission
    ...
    %% Following Equation (4), (5), (6) to find A_{H}, A_{F} and A_{L}
        for seg=1:1:length(pack_tx_segments(Transmit,:))
            ...
            if(seg~=Header_N_DR9+1)
                clear active_pattern
                clear iscollision

                if(seg~=length(pack_tx_segments(Transmit,:)) && seg<=Header_N_DR9)         % collisions during header transmission
                    
                transmission_sim_header(m,seg) = length(find(seg_simultaneous(:,1:Header_N_DR9)>=(pack_tx_segments(Transmit,seg)-Header_duration) & seg_simultaneous(:,1:Header_N_DR9)<=(pack_tx_segments(Transmit,seg)+Header_duration)));
                transmission_sim_frag(m,seg) = length(find(seg_simultaneous(:,(Header_N_DR9+1):end-1)>=(pack_tx_segments(Transmit,seg)-F_duration) & seg_simultaneous(:,(Header_N_DR9+1):end-1)<=(pack_tx_segments(Transmit,seg)+Header_duration)));
                ...
                elseif(seg~=length(pack_tx_segments(Transmit,:)) && seg>Header_N_DR9)    % collisions during payload data fragments transmission
                    
                transmission_sim_header(m,seg) = length(find(seg_simultaneous(:,1:Header_N_DR9)>=(pack_tx_segments(Transmit,seg)-Header_duration) & seg_simultaneous(:,1:Header_N_DR9)<=(pack_tx_segments(Transmit,seg)+F_duration)));
                transmission_sim_frag(m,seg) = length(find(seg_simultaneous(:,(Header_N_DR9+1):end-1)>=(pack_tx_segments(Transmit,seg)-F_duration) & seg_simultaneous(:,(Header_N_DR9+1):end-1)<=(pack_tx_segments(Transmit,seg)+F_duration)));
                ...
                transmission_sim_header(m,seg) = length(find(seg_simultaneous(:,1:Header_N_DR9)>=(pack_tx_segments(Transmit,seg)-Header_duration) & seg_simultaneous(:,1:Header_N_DR9)<=(pack_tx_segments(Transmit,end))+Last_fragment_duration));
                transmission_sim_frag(m,seg) = length(find(seg_simultaneous(:,(Header_N_DR9+1):end-1)>=(pack_tx_segments(Transmit,seg)-F_duration) & seg_simultaneous(:,(Header_N_DR9+1):end-1)<=(pack_tx_segments(Transmit,end))+Last_fragment_duration));
...
%                      Coordinates(:,2) = sin(Theta).*rho;
                      index = randi(length(Grid),length(iscollision),1);
...
%                     Location_Nodes_Int = sqrt((Interfering_Nodes(:,1)-Grid(Satellite_subpoint,1)).^2 + (Interfering_Nodes(:,2)-Grid(Satellite_subpoint,2)).^2)';
                      Location_Nodes_Int = sqrt((Interfering_Nodes(:,1)-Satellite_Link_Farms(c,Satellite_subpoint,1)).^2 + (Interfering_Nodes(:,2)-Satellite_Link_Farms(c,Satellite_subpoint,2)).^2)';
...
%                      Location_Nodes_Int = sqrt(Coordinates(:,1).^2 + Coordinates(:,2).^2)';
                     ...
                     %% Rician fading for interfering signals
                        ...
                        %% total interferance = sum of all the interfering signals 
                           ...
                           % add underground path loss 
                           pr_h_g_I = sum(Pt.*h1C.*Gr.*Gt./(10.^(U2Aloss(RealSoilDielectric, ImagSoilDielectric,Depth,dPropogation,eta,Freq_Band)./10)));
...
%                        pr_h_g_D = Pt.*h1D.*Gr.*Gt./U2Aloss(RealSoilDielectric, ImagSoilDielectric,Depth,Distance(c),eta,Freq_Band);
                         pr_h_g_D = Pt.*h1D.*Gr.*Gt./(10.^(U2Aloss(RealSoilDielectric, ImagSoilDielectric,Depth,Distance(c),eta,Freq_Band)./10));
...
Success_header = Header_N_DR9 - length(nonzeros(target_collided(1:Header_N_DR9)));       % No. of successfully received headers
Threshold = size(pack_tx_segments,2) - round(fragment_PHY_length *(1-Code_Rate))-Header_N_DR9 - length(Transceiver_wait);
Success_fragment = size(target_collided,2) - length(nonzeros(target_collided((Header_N_DR9+2):end)))-Header_N_DR9-1;
...
Success_header_capture =Header_N_DR9 - length(nonzeros(target_discarded(1:Header_N_DR9)));

        if (Success_header_capture>=1)
        Success_fragment_capture = size(target_discarded,2) - length(nonzeros(target_discarded(Header_N_DR9+2:end)))-Header_N_DR9-1;
...
PS_DR9(c)=decoded;   %Simulated overall success probability
%PH_DR9(c,m)=H_success; %Simulated success probability of headers
%PF_DR9(c,m)=F_success; %Simulated success probability of data fragments

PS_DR9_Capture(c)=decoded_Capture; %Simulated success probability with capture effect

[PS_DR9_analytical,PH,PF] = DR9_analytical (N,pkct_p_h,Header_duration,F_duration,Last_fragment_duration,fragment_length,Header_N_DR9,Threshold,OBW_channels);
PA_S(c) = PS_DR9_analytical;  %Analytical overall success probability
...
% hold on
% h(2)=plot(Distance/1e3,PS_DR9/MonteCarlo,'b*');
% h(3)=plot(Distance/1e3,PS_DR9_Capture/MonteCarlo,'ro-');
% hold off
% grid on
...
% legend('DR9 analytical','DR9 Simulated', 'DR9 Capture effect')
figure
h(1)=plot(Distance/1000,PA_S.*P_snr(:,1)','b-');
hold on
h(2)=plot(Distance/1000,PS_DR9/MonteCarlo.*P_snr(:,1)','b*');
hold on
h(3)=plot(Distance/1000,PS_DR9_Capture/MonteCarlo.*P_snr(:,1)','ro-');
hold on
plot(Distance/1000,P_snr(:,1)','-k','LineWidth',2);
hold off
grid on
ylabel('Success probability', 'Interpreter', 'Latex');
xlabel('Distance from node to satellite (km)', 'Interpreter', 'Latex');
axis([Distance(1)/1000 Distance(end)/1000 0 1])
legend('DR6 analytical','DR6 Simulated', 'DR6 Capture effect','Psnr',Location='southeast')
toc
% figure
% PDR_Capture=PS_DR9_Capture/MonteCarlo;
% EPP_Capture=EPP(ToA_DR9(1),PDR_Capture);
% plot(Distance/1000,EPP_Capture,'-k','LineWidth',2);
% hold on
% grid on
% ylabel('EPP (J)','Interpreter','Latex','FontSize', 12);
